...
export default function Invoice({navigation,route}) {
    ...
    const [dVoucher,setdVoucher] = useState('')
    ...
    const ViewCalendar=()=>{
            ...
            <Modal visible={openViewCalendar} animationType="fade" transparent={true}> 
                <View style={[style.centeredView,style.modalView,{marginTop:200,backgroundColor:'white'}]}>
                    <TouchableOpacity onPress={()=>{
                        setopenViewCalendar(false);
                        setdVoucher('');    
                    }} style={{flexDirection:'row', gap:10, paddingTop:10,paddingBottom:10}}>
                        <Icon 
                        name='window-close'
                        size={25}
                        />
                        <Text style={{fontSize:15}}>Đóng và không chọn ngày</Text>
                    </TouchableOpacity>
        ...
        await GetSalesVoucher(dayVoucher,voucher,custumer,setvisibleLoadData).then((data)=>{
            if(data.status==200){
                ...
                if(data.data.ObjectData.length > 0) {
                    ...
                    let dataAfterSort = clsFunc.fIncSort(data.data.ObjectData,'VoucherNo');
          
                    setData(dataAfterSort);
                    setFilteredData(dataAfterSort); 

                    // Gọi hàm tính tổng cộng cho 1 dòng
                    const totals = clsFunc.fAddTotalRowToData(data.data.ObjectData, titleHeaderComponent.id);
                    setTotalRow(totals);
        ...
        await GetSalesVoucher(vDate,VoucherNo,TradeName,setvisibleLoadData).then((data)=>{
            if(data.status==200){
                if(data.data.ObjectData.length > 0) {
                    ...
                    setdatadetail(dataAfterSort);

                    // Gọi hàm tính tổng cộng cho dòng TotalRow
                    const totals = clsFunc.fAddTotalRowToData(data.data.ObjectData, titleHeaderComponent.id);
                    setTotalRow(totals);
    ...
    const ModalPhieuGiaoHang = ({ modalPhieugiaohang, setmodalPhieugiaohang, datadetail, itemselect }) => {
        ...
        console.log('datadetail được gọi trong modal: ',datadetail)

        useEffect(() => {
            let sum = 0;
            for (let i = 0; i < datadetail.length; i++) {
                if(datadetail[i].ItemName !== 'TỔNG CỘNG') {
                    sum += datadetail[i].ConvertAmount;
                } 
            }
            ...
            <Modal visible={modalPhieugiaohang} animationType="slide" transparent={true}>
                <View style={{...ModalNewStyle.modalOverlay}}>
                    <View style={{...ModalNewStyle.modalContainer}}>
                        ...
                        <Text style={style.subTitle}>NV: {itemselect.PrepairedByName} - Ngày: {moment(itemselect.VoucherDate).format("DD/MM/YYYY")}</Text>
                                    ...
                                    <View style={{...containerView('report',data),margin:0}}>
                                        ...
                                        <FlatList
                                            ...
                                            ListFooterComponent={() =>
                                                ...
                                                <View style={{...GridStyle('').dataRow}}>
                                                    {keys.map(key => (
                                                        <Text key={key} style={{...GridStyle(key).dataCell}}>
                                                            {clsFunc.fFormatDataItem(key, totalRow)}
                                                        </Text>
                                                    ))}
                                                </View>
                                                ) : null
                        ...
                        <Text style={style.total}>Tổng cộng: {tongtien!=0?tongtien.toString().replace(/\B(?=(\d{3})+(?!\d))/g,'.'):'Chưa tính'}</Text>
                        <Text style={style.note}>Lưu ý: Hàng đã mua không được trả lại</Text>
